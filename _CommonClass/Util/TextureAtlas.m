...
@synthesize totalQuads = totalQuads_, capacity = capacity_, blendType = blendType_;
...
-(void)updateQuad:(TQuadInfo*)quad atIndex:(NSUInteger)n
{
...
-(void)insertQuadFromIndex:(NSUInteger)oldIndex atIndex:(NSUInteger)newIndex
{
	...
	// tex coordinates
	TQuadInfo quadsBackup = quads_[oldIndex];
...
{
	if(newCapacity == capacity_) return YES;
...
#define kQuadSize sizeof(quads_[0].bl)
	...
	// vertices
	glVertexPointer(2, GL_FLOAT, kQuadSize, (void*) offsetof( TVertexInfo, vertices));
	
	// colors
	glColorPointer(4, GL_UNSIGNED_BYTE, kQuadSize, (void*) offsetof( TVertexInfo, colors));
	
	// tex coords
	glTexCoordPointer(2, GL_FLOAT, kQuadSize, (void*) offsetof( TVertexInfo, texCoords));
	...
	// vertex
	int diff = offsetof( TVertexInfo, vertices);
	glVertexPointer(2, GL_FLOAT, kQuadSize, (void*) (offset + diff) );

	// color
	diff = offsetof( TVertexInfo, colors);
	glColorPointer(4, GL_UNSIGNED_BYTE, kQuadSize, (void*)(offset + diff));
	
	// tex coords
	diff = offsetof( TVertexInfo, coords);
	glTexCoordPointer(2, GL_FLOAT, kQuadSize, (void*)(offset + diff));

	switch(blendType_)
	{
		case BT_NORMAL:
			if(texture_.pixelFormat == kTexture2DPixelFormat_RGBA8888) glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
