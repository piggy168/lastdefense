...
- (void)blobSend_FailureDelegate:(id)status{
	OFCloudStorageStatus_Code statusCode = [status getStatusCode];

	// [status isAnyError] is a general test for error status.
	OFAssert([status isAnyError], @"Failure delegate handles errors, never handles CSC_Ok.");

	switch (statusCode) {
		case CSC_GatewayTimeout:
			statusTextBox.text = @"not reachable";
			// [status isNetworkError] is another, more general test for network errors.
			OFAssert([status isNetworkError], @"CSC_GatewayTimeout is a network error.");
			break;
...
- (void)blobFetch_FailureDelegate:(id)status{
	OFCloudStorageStatus_Code statusCode = [status getStatusCode];
